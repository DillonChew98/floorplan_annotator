...
public:
    ...
    bool colorize, bool postprocess, const std::string& outputfilepath)
  {
    ...
    try {
      ...
      std::pair<std::ifstream, std::string> data = std::make_pair(std::ifstream{filepath, std::ios::binary}, filepath);
  ...
  std::pair<std::string, std::string> generate_multiform_data(std::pair<std::ifstream, std::string> & file_data,
...
bool RestCommunicator::send_update(const std::string & filepath, 
